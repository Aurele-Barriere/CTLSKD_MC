...
Finally, the number of states in the augmented model is easily computable: $|S|\times 2^{|S|-1}\times |\mathcal{O}|$ (we only build the states $(s,I,o)$ wher $s\in I$, as the others are not reachable). This explains why the time of model-checking \ctlskd\ explodes so quickly in Figure~\ref{time_ctlskd}.
